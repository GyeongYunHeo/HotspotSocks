...
final class HttpProxyServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.example.HotspotSocks.http-server")
    ...
    private var pacFallbackHost: String?
    ...
    private func configure(_ listener: NWListener) {
        listener.stateUpdateHandler = { [weak self, weak listener] state in
            ...
            case .ready:
                stateHandler(.ready)
                AppLogger.http.info("HTTP proxy listener ready on port \(listener.port?.rawValue ?? 0, privacy: .public)")
    ...
    private func accept(_ connection: NWConnection) {
        ...
        let session = HttpProxySession(
            ...
            pacFallbackHost: pacFallbackHost,
            proxyPort: listener?.port?.rawValue ?? 9877,
    ...
    private func publishStatistics() { statisticsHandler(statistics.snapshot()) }
}
